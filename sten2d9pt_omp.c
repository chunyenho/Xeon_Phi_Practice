...
void
stencil9pt_omp(REAL *finp, REAL *foutp, 
                ...
                REAL ctr, REAL next, REAL diag,int count)
{
  ...
  REAL *fout = foutp;
  int i,x,y;
#pragma offload target(mic) in(fin:length(WIDTHP*HEIGHT)) inout(fout:length(WIDTHP*HEIGHT))
{
  for (i=0; i<count; i++) {
...
#pragma omp master
  ...
  stencil9pt_omp(fin, fout, width, height, 
